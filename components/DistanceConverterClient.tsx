...
export default function DistanceConverterClient() {
  ...
  const convertDistance = (value: number, fromUnitValue: string, toUnitValue: string) => {
    ...
    const metersPerUnit: {[key: string]: number} = {
      'millimeters': 0.001,
      'centimeters': 0.01,
      'meters': 1,
      'kilometers': 1000,
    ...
    if (result >= 1000000) {
      formattedResult = `${(result / 1000000).toFixed(2)}M ${t(`units.${toUnitValue}`)}`;
    } else if (result >= 1000) {
  ...
  const config: CalculatorConfig = useMemo(() => ({
    ...
    input1: {
      ...
      unit: fromUnit,
      units: [
        {value: 'millimeters', label: t('units.millimeters')},
        {value: 'centimeters', label: t('units.centimeters')},
        {value: 'meters', label: t('units.meters')},
        {value: 'kilometers', label: t('units.kilometers')},
        {value: 'inches', label: t('units.inches')},
        {value: 'feet', label: t('units.feet')},
        {value: 'yards', label: t('units.yards')},
        {value: 'miles', label: t('units.miles')}
      ],
      onChange: setValue,
